...
for time_step in range(num_time_steps):
    ...
    # old measurement
    old_observations = np.array(model_manager.batch_measurements).reshape(
        [model_manager.n_batches * model_manager.batch_size, model_manager.num_dims])[predictions_mask]
...
#
print(
    'this is the base mode, now you can execute and test everything as if you were in a normal python shell, that executed all commands until now!')
...
def solve_matching_problem(dataset, model_manager):
    ...
    for it in range(TIME_STEPS):
        ...
        current_predictions_real = list(
            filter(lambda x: x[0] >= 0, current_predictions_real))  # TODO this is only a heuristic and no guarantee!!!
        ...
        last_tenth_prediction_idxss = list(
            filter(lambda x: SIZE_X - current_predictions_real[x][0] <= X_DETECTION_TOLERANCE,
                   range(num_predictions_real)))
        ...
        first_tenth_measurement_idxss = list(
            filter(lambda x: current_measurements_real[x][0] <= X_DETECTION_TOLERANCE, range(num_measurements_real)))
        ...
        for i in range(len(measurement_idxs)):
            if measurement_idxs[i] < num_measurements_real and prediction_idxs[i] < num_predictions_real:
                ...
                try:
                    ...
                    trajectories[trajectory_id] = [[current_measurements[first_tenth_measurement_idxss[
                        prediction_idxs[i] - num_predictions_real]]], [it]]
                except Exception as exp:
