	...
	def associate_data(self):
		...
		for time_step in range(self.global_config['num_timesteps']):
			...
			if old_measurements is not None:
				...
				for idx, prediction_id in enumerate(prediction_ids):
					if old_measurements[prediction_id][1]:
						plt.scatter([old_measurements[prediction_id][0][0]], [old_measurements[prediction_id][0][1]], c='cyan') # , label='old measurement')
					else:
						plt.scatter([old_measurements[prediction_id][0][0]], [old_measurements[prediction_id][0][1]], c='yellow') # , label='old measurement artificial')
			...
			if prediction_values != []:
				plt.scatter(np.array(prediction_values)[:, 0], np.array(prediction_values)[:, 1], c='red', label='prediction')

			#
			distance_matrix = np.inf * np.ones([2 * len(measurements) + len(prediction_values), 2 * len(prediction_values) + len(measurements)])
			#
			...
			for idx in range(len(measurement_idxs)):
				if measurement_idxs[idx] < len(measurements) and prediction_idxs[idx] < len(prediction_values):
					...
					#
					line = np.stack((measurements[measurement_idxs[idx]], prediction_values[prediction_idxs[idx]]), axis=0)
					...
					pseudo_measurement = np.array([prediction[0] + self.global_config['distance_threshhold'], prediction[1]])
					...
					plt.gcf().gca().add_artist(circle)
					#
					pseudo_prediction = np.array([measurement[0] + self.global_config['distance_threshhold'], measurement[1]])
