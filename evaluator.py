...
class Evaluator(object):
    ...
    def assigment_of_measurement_in_particle(self, timestep, measurement):
        for idx, particle_list in enumerate(self.particles):
            for particle in particle_list:
                ...
                if timestep == particle_timestep and particle_measurement[0] == measurement[0] and particle_measurement[
                    1] == measurement[1]:
                    logging.debug(
                        str(timestep) + ' - ' + str(measurement) + ' - measurement found match in particles! - ' + str(
                            idx))
                    return idx
        logging.debug(str(timestep) + ' - ' + str(
            measurement) + ' - measurement found no match in particles! Probably a artificial measurement.')
    ...
    def error_of_first_kind(self):
        ...
        for track_id in list(self.tracks.keys()):
            ...
            if is_incremented and self.global_config['debug']:
                logging.debug('in error_of_first_kind')
                code.interact(local=dict(globals(), **locals()))
    ...
    def assigment_of_measurement_in_track(self, timestep, measurement):
        for idx in list(self.tracks.keys()):
            ...
            for it, track_measurement in enumerate(track.measurements):
                ...
                if track_timestep == timestep and track_measurement[0] == measurement[0] and track_measurement[1] == \
                        measurement[1]:
                    logging.debug(
                        str(timestep) + ' - ' + str(measurement) + ' - measurement found match in particles! - ' + str(
                            idx))
    ...
    def error_of_second_kind(self):
        ...
        logging.info('ratio_error_of_second_kind: ' + str(ratio_error_of_second_kind))
