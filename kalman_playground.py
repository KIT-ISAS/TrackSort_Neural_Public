...
def kalman_playground(aligned_track_data, normalization_constant=1):
    ...
    cv_model = CV_Model(name = "CV_Model", dt=0.01, s_w=18, s_v=3.6E-7, default_state_options = {})
    # Test CV model
    c = 0
    error = 0
    for track in aligned_track_data:
        cv_state = CV_State(track[0], velocity_guess=0.1, velo_var_x=0.13, velo_var_y = 0.5, pos_var=3.6E-7)
        ...
        for i in range(1,track.shape[0]):
            ...
            if np.array_equal(pos,np.array([0, 0])) == False:
                ...
                error = error + (np.abs(pos[0]-state_pos[0]) + np.abs(pos[1]-state_pos[1]))/2
                # update
